...
export class IconButtonDirective {
  ...
  readonly classes = computed(() => {
    const baseClasses = 'flex items-center justify-center relative rounded-lg tracking-wide transition-all duration-200 cursor-pointer disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 [&_base-icon]:stroke-current [&_base-icon]:fill-current';
    ...
    const sizeMap: Record<IconButtonSize, string> = {
      ...
      md: 'h-8! w-8! min-w-8! max-w-8! [&_base-icon]:w-4! [&_base-icon]:h-4!',
