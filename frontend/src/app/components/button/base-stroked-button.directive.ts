...
export class StrokedButtonDirective {
  ...
  readonly classes = computed(() => {
    const baseClasses = 'flex items-center gap-2 relative rounded-lg border text-center justify-center tracking-wide transition-all duration-200 cursor-pointer disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 [&_base-icon]:stroke-current [&_base-icon]:fill-current';
    ...
    const colorMap: Record<StrokedButtonColor, string> = {
      ...
      transparent: 'text-slate-700 dark:text-slate-200 border-transparent hover:bg-slate-100 dark:hover:bg-slate-800 active:bg-slate-200 dark:active:bg-slate-700',
    };
